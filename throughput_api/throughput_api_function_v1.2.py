...
def beginThroughputTest(req):
    ...
    total_bandwidth = 0
    Env = testEnvironment()
    if Env == True:  # 检测是否安装iperf3
        if temp[test_id][0] == '-c':
            # 检测server端5201端口和5202端口是否开启
            try:
                server1 = testServerOpen(temp[test_id][2], "5201")
                server2 = testServerOpen(temp[test_id][2], "5202")
            except Exception as e:
                print(e)
                exit()
            value_len = len(temp[test_id])
            if value_len > 5:
                T = int(temp[test_id][5])
            else:
                T = 10
            if server1 and server2:
                for i in range(T):
                    if flag == 1:
                        t1 = ThreadWithReturnValue(target=uploadTest, args=(temp, test_id,))
                        t2 = ThreadWithReturnValue(target=downloadTest, args=(temp, test_id,))
                        t1.start()
                        t2.start()
                        res1 = t1.join()
                        res2 = t2.join()
                        if res1["code"] == 1 and res2["code"] == 1:
                            if i == 0:
                                total_bandwidth = addTwoBw(res1["data"]["total_bandwidth"],
                                                           res2["data"]["total_bandwidth"])  # 带单位
                                avg_total_bandwidth = avgTwoBw(res1["data"]["avg_total_bandwidth"],
                                                               res2["data"]["avg_total_bandwidth"])  # 带单位

                            else:
                                total_bandwidth = avgTwoBw(total_bandwidth, addTwoBw(res1["data"]["total_bandwidth"],
                                                                                     res2["data"]["total_bandwidth"]))
                                avg_total_bandwidth = avgTwoBw(avg_total_bandwidth,
                                                               avgTwoBw(res1["data"]["avg_total_bandwidth"],
                                                                        res2["data"]["avg_total_bandwidth"]))
                            with open(test_id + '.log', "a") as f:
                                f.write('时间戳：' + str(time.time()) + '\n')
                                f.write(res1["data"]["response"])
                                f.write(res2["data"]["response"])
                                f.write('avg_total_bandwidth=' + str(avg_total_bandwidth) + '\n')
                                f.write('total_bandwidth=' + str(total_bandwidth) + '\n')
                                f.flush
                                time.sleep(interval)
                            for x in range(len(res1["data"]["bandwidth"])):
                                    res = {
                                        "code": 1,
                                        "msg": "测试成功",
                                        "current_wifi": "xxx",
                                        "data": {
                                            "id_of_time": test_id,
                                            # "string_type": "上传",
                                            "bandwidth": addTwoBw(res1["data"]["bandwidth"][x], res2["data"]["bandwidth"][x]),
            ...
            while flag == 1:
                process = subprocess.Popen(
                    ["iperf3", "-s", temp[test_id][1], "-p", temp[test_id][8], "-i", temp[test_id][6]],
                    stdout=subprocess.PIPE
                )
                response, _ = process.communicate()
                first_re = re.findall('SUM.+/sec', response.decode('utf-8'))
                re_list = re.findall('([\d.]+) *[A-Za-z.]+/sec', response.decode('utf-8'))
                bandwidth_list = []
                sender = re.findall('([0-9.]+) bits.+sender', response.decode('utf-8'))[0]
                if float(sender) <= 0:
                    string_type = "上传"
                else:
                    string_type = "下载"
                if first_re and re_list:
                    for parallel in range(int(temp[test_id][3])):
                        bandwidth_list.append(re_list[parallel])
                    if index == 0:  # 初次的平均带宽和总带宽
                        ...
                        total_bandwidth = (total_bandwidth + float(
                            re.findall('([\d.]+) *[A-Za-z.]+/sec', first_re[0])[0])) / 2
                    index += 1
                    with open(test_id + '.log', "a") as f:
                        f.write('时间戳：' + str(time.time()) + '\n')
...
async def send_msg(websocket):
    # logfile = open('200' + '_res.log', "r")
    with open('400' + '_res.log', "r") as logfile:
...
t1 = threading.Thread(target=beginThroughputTest, args=())
t1.start()
